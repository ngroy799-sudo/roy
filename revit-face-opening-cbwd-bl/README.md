...
1. 把本資料夾放到本機，例如：`F:\For Cursor\revit-face-opening-cbwd-bl`
2. Revit 2023 開啟專案模型
...
| SelectionMode | `SelectedOnly` 或 `AllOpenings` |
| FamilyNameContains | 預設 `Face Opening` |
| ParameterName | 預設 `CBWD B.L.` |
| ReportPath | `(auto)` |

6. 若用 `SelectedOnly`：先在 Revit 選好 Face Opening，再按 **Play**
7. 看 Outputs：`Summary` / `SuccessCount` / `Warnings` / `Status`
