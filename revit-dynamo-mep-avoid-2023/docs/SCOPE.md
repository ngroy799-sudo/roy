...
在 Revit 2023 用 Dynamo：對 **Pipe／Duct** 偵測與 **已點選障礙模型** 的衝突；**不移動障礙**；只 **向下 Fitting** 繞行；**含保溫**外表面垂直淨空 **≥ 10 cm**；兩端接回原標高；空間不足則跳過並記錄。
...
| 代號 | 含義 | 本專案用法 |
|------|------|------------|
| **Subject（MEP）** | 被繞行改線的機電曲線元素 | 第一版：**Pipe、Duct**（Tray/Conduit → Phase 3） |
